...
# Get current LiteLLM deployment
...
# Patch the deployment to add Langfuse env vars
kubectl set env deployment/litellm -n default \
  --from=secret/langfuse-credentials \
  LANGFUSE_PUBLIC_KEY \
  LANGFUSE_SECRET_KEY \
  LANGFUSE_HOST
